...
function renderSettings(){
  ...
  $("#logout").onclick = ()=>{ state.session=null; if(typeof clearSession==="function") clearSession(); state.view="login"; render(); };
...
async function init(){
  try{
    ...
    // Restore "Keep me logged in" session (or this tab's session) so a
    // refresh doesn't bounce a signed-in resident back to the login screen.
    // Only trust it if that member still actually exists.
    const restored = (typeof loadSession === "function") ? loadSession() : null;
    if(restored && state.members.some(m=>m.username===restored.username)){
